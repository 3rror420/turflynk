...
function explicitQuoteOption(value) {
...
function buildQuotePayload(form) {
  ...
  payload.obstacles_list = checkedValues('obstacles_list', form).filter((value) => value && value !== 'none');
  ...
  payload.fenced = explicitQuoteOption(payload.fenced);
  payload.overgrown = explicitQuoteOption(payload.overgrown);
  payload.obstacles = explicitQuoteOption(payload.obstacles) || payload.obstacles_list.length > 0;
  payload.rushJob = explicitQuoteOption(payload.rushJob);
  payload.limitedAccess = explicitQuoteOption(payload.limitedAccess);
  payload.slopedTerrain = explicitQuoteOption(payload.slopedTerrain);
  payload.denseVegetation = explicitQuoteOption(payload.denseVegetation);
  payload.gates = explicitQuoteOption(payload.gates);
  payload.gateHandling = explicitQuoteOption(payload.gateHandling);
...
function drawParcel(geometry, properties = {}) {
  ...
  const parcelGeoJsonClone = cloneGeoJsonSafe(feature);
  state.selectedParcelGeoJson = cloneGeoJsonSafe(parcelGeoJsonClone);
  state.selectedParcelGeoJSON = cloneGeoJsonSafe(parcelGeoJsonClone);
...
function metaPixelKeyPart(value) {
  return String(value || '')
    .trim()
...
function metaPixelNumber(value, fallback = null) {
  const n = Number(value);
  return Number.isFinite(n) && n > 0 ? n : fallback;
}

function trackLeadForFreshEstimate(payload = {}, signature = '') {
  const value = metaPixelNumber(payload.estimate);
  const keySeed = payload.id || payload.quote_id || [
    signature,
    value || '',
    Math.round(Number(payload.mowAreaSqft || 0)),
    Math.round(Number(payload.lotAreaSqft || 0)),
  ].join('|');
  const storageKey = `mownwa_meta_lead_tracked_${metaPixelKeyPart(keySeed)}`;
  try {
    if (sessionStorage.getItem(storageKey)) return;
    const params = {
      content_name: 'Lawn Quote Generated',
      content_category: 'Instant Quote',
      currency: 'USD',
    };
...
function applyDefaultQuoteService() {
  const select = byId('quoteServiceSelect');
  if (!select) return;
  if (Array.from(select.options).some((option) => option.value === 'mowing')) {
    select.value = 'mowing';
  }
}
...
// friendlyAdminSaveError, hydrateRegionEditor → public/js/admin/admin-ui.js
...
function exitParcelSelectMode() {
  ...
  setEditorModeLabel('Mode: Ready');
  _setQuoteMapPointerEnabled(false);
  byId('selectParcelBtn')?.classList.remove('active');
  byId('useThisParcelBar')?.classList.add('hidden');
}

function refreshConfirmParcelDisplay(options = {}) {
  const data = options.lookupData || {};
  const normalized = options.normalized || data.normalized || {};
  const attrs = options.attrs || normalized.attributes || {};
  const parcelProps = options.parcelProps || parcelPropertiesFromLookupData(data);
  const quoteForm = byId('quoteForm');
  const previewFeature = options.previewFeature || null;
  const address = firstTextValue(
    options.address,
    options.addressLabel,
    propText(parcelProps, 'adrlabel', 'situsaddress', 'siteaddress', 'propertyaddress', 'address'),
    [normalized.address, normalized.city].filter(Boolean).join(', '),
  ...
  const lotSqft = Number(options.lotSqft || normalized.areaSqft || propText(parcelProps, 'areaSqft') || 0)
    || (previewFeature ? layerAreaSqFt(previewFeature) : 0)
    || (options.useFormAreas ? Number(quoteForm?.elements?.lotAreaSqft?.value || 0) : 0);
  const mowSqft = Number(options.mowSqft || 0)
    || (options.useFormAreas ? Number(quoteForm?.elements?.mowAreaSqft?.value || 0) : 0);

  renderConfirmProperty({
    headline: options.headline || 'Parcel selected. Boundary loaded.',
    parcelProps,
    normalized,
    attrs,
  ...
  updateMowAreaHelper(lotSqft, mowSqft);
  console.log('[Confirm Parcel Refresh]', { address, parcelId, owner, lotSqft });
  return { address, parcelId, owner, county, lotSqft, mowSqft };
}

function resetCommittedParcelAliases() {
  [
...
function cloneGeoJsonSafe(value) {
  if (!value) return null;
  if (typeof value === 'string') {
    try { return JSON.parse(value); } catch (err) { console.warn('[GeoJSON Clone Failed]', err); return null; }
  }
  try { return JSON.parse(JSON.stringify(value)); } catch (err) { console.warn('[GeoJSON Clone Failed]', err); return value; }
}

function parcelGeometryFromGeoJson(value) {
...
function hasValidParcelGeoJson(value) {
  const geometry = parcelGeometryFromGeoJson(value);
  return (geometry?.type === 'Polygon' || geometry?.type === 'MultiPolygon')
    && Array.isArray(geometry.coordinates)
    && geometry.coordinates.length > 0;
}

function commitSelectedParcelForQuote(data = {}, options = {}) {
  const geometry = options.geometry || data.feature?.geometry || null;
  const normalized = options.normalized || data.normalized || {};
  const attrs = options.attrs || normalized.attributes || {};
  const parcelProps = options.parcelProps || parcelPropertiesFromLookupData(data);
  let parcelGeoJson = geometry
    ? esriPolygonToGeoJSONFeature(geometry, { ...parcelProps, role: 'parcel' })
    : cloneGeoJsonSafe(options.parcelGeoJson || data.parcelGeoJSON || data.parcelGeoJson || data.feature || null);
  parcelGeoJson = cloneGeoJsonSafe(parcelGeoJson);

  if (!hasValidParcelGeoJson(parcelGeoJson)) {
    console.warn('[Confirm Parcel Commit] missing parcel geometry', { hasGeometry: Boolean(geometry), hasGeoJson: Boolean(parcelGeoJson) });
    return null;
  }

  try { if (typeof turf !== 'undefined') turf.rewind(parcelGeoJson, { mutate: true }); } catch {}
  ...
  const addressParts = normalizeServiceAddressParts(options.serviceAddress || {});
  const address = firstTextValue(
  ...
  const lotSqft = Number(options.lotSqft || normalized.areaSqft || propText(parcelProps, 'areaSqft') || 0)
    || layerAreaSqFt(parcelGeoJson)
    || 0;
  const lookupPoint = options.lookupPoint
    || data.lookupPoint
  ...
  if (!hasValidParcelGeoJson(parcelGeoJsonClone) || !parcelGeometryClone) {
    console.warn('[Confirm Parcel Commit] invalid cloned parcel geometry', { hasGeoJson: Boolean(parcelGeoJsonClone) });
    return null;
  }
  const quoteForm = byId('quoteForm');

  state.parcelGeometry = geometry || parcelGeometryClone;
  state.parcelFeature = parcelGeoJsonClone;
  state.parcelLayer = parcelGeoJsonClone;
  state.parcelProperties = parcelProps;
  state.selectedParcel = parcelGeoJsonClone;
  ...
  if (quoteForm) {
    ...
    if (quoteForm.elements.lotAreaSqft) quoteForm.elements.lotAreaSqft.value = lotSqft || '';
    if (quoteForm.elements.mowAreaSqft) quoteForm.elements.mowAreaSqft.value = '';
    if (quoteForm.elements.customerAdjustedMowableSqft) quoteForm.elements.customerAdjustedMowableSqft.value = '';
    if (quoteForm.elements.lotSource) quoteForm.elements.lotSource.value = '';
  }

  if (lookupPoint?.lat && lookupPoint?.lng) {
    setLatLng(lookupPoint.lat, lookupPoint.lng);
  ...
  if (state.map) {
    withMapReady(() => {
      fitLayerBoundsWithContext(parcelGeoJsonClone);
      reorderMapOverlays();
      requestAnimationFrame(() => state.map?.resize?.());
    });
  }

  console.log('[Confirm Parcel Commit]', { address, parcelId, lotSqft, hasGeoJson: !!parcelGeoJsonClone });
  return { address, parcelId, lotSqft, parcelGeoJson: parcelGeoJsonClone };
}
...
async function applyParcelFromClick(latlng) {
  ...
  const preview = refreshConfirmParcelDisplay({
    lookupData: data,
    parcelProps,
    normalized: data.normalized || {},
    attrs: data.normalized?.attributes || {},
    previewFeature: state.pendingParcelPreviewFeature,
    headline: 'Parcel selected. Confirm to use this property.',
    method: data.method || '',
    mowSqft: 0,
  });
  ...
  if (bar && addrEl) {
    addrEl.textContent = preview.address || 'Parcel found';
    bar.classList.remove('hidden');
  }
}

async function confirmParcelSelection(options = {}) {
  ...
  state.pendingQuote = null;
  state.lastQuote = null;
  const commit = commitSelectedParcelForQuote(data, {
    geometry,
    normalized,
    attrs,
    parcelProps,
  ...
  refreshConfirmParcelDisplay({
    ...
    serviceAddress: parcelAddress,
    method: data.method || '',
    parcelId: commit?.parcelId || normalized.parcelId || propText(attrs, 'parcelid', 'parcel_id', 'PARCELID', 'PIN') || '',
    county: normalized.county || propText(attrs, 'countyid', 'county', 'COUNTY') || '',
    lotSqft: commit?.lotSqft || (quoteForm ? Number(quoteForm.elements.lotAreaSqft.value || 0) : 0),
    mowSqft: quoteForm ? Number(quoteForm.elements.mowAreaSqft.value || 0) : 0,
  });
  showPropertyConfirmationPanel(true);
  exitParcelSelectMode();
  showQuoteFlowStep(options.nextStep || 'property');
}
...
function handleSelectDifferentParcelClick(event) {
  event.preventDefault();

  const form = byId('quoteForm');
  const hasCoords = Boolean(Number(form?.elements?.lat?.value || 0) && Number(form?.elements?.lng?.value || 0));
  const hasAddress = ['address', 'city', 'zip'].some((name) => (form?.elements?.[name]?.value || '').trim());
  const hasParcelContext = Boolean(
    state.parcelLayer ||
    state.parcelFeature ||
    state.selectedParcel ||
    state.pendingParcelFeature ||
  ...
  if (hasCoords || hasAddress || hasParcelContext) {
    ...
    requestAnimationFrame(() => {
      state.map?.resize?.();
      if (state.parcelFeature) fitLayerBoundsWithContext(state.parcelFeature);
    });
  } else {
